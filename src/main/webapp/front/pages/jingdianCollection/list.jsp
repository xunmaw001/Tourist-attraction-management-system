            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">景点收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="jingdianName" id="jingdianName" placeholder="景点名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="jingdianTypes" id="jingdianTypes" lay-filter="jingdianTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jingdianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="jingdianDengjiTypes" id="jingdianDengjiTypes" lay-filter="jingdianDengjiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jingdianDengjiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">景点特色</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jingdianTese" id="jingdianTese" placeholder="景点特色" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../jingdian/detail.jsp?id='+item.jingdianId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.jingdianPhoto?item.jingdianPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../jingdian/detail.jsp?id='+item.jingdianId)"><span>{{item.jingdianName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteJingdianCollection(item.id)">删除 {{item.jingdianName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jingdianCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jingdianCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteJingdianCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jingdianCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jingdianName').val() != null && jquery('#jingdianName').val() != "" && jquery('#jingdianName').val() != "null" ) {
                param['jingdianName'] = jquery('#jingdianName').val();
            }
                 
            var jingdianTypes = $("#jingdianTypes").find("option:selected").val();
            if (jingdianTypes != null && jingdianTypes != "" && jingdianTypes != "null") {
                            param['jingdianTypes'] = jingdianTypes;
                        }
                 
            var jingdianDengjiTypes = $("#jingdianDengjiTypes").find("option:selected").val();
            if (jingdianDengjiTypes != null && jingdianDengjiTypes != "" && jingdianDengjiTypes != "null") {
                            param['jingdianDengjiTypes'] = jingdianDengjiTypes;
                        }
                                 
            if (jquery('#jingdianTese').val() != null && jquery('#jingdianTese').val() != "" && jquery('#jingdianTese').val() != "null" ) {
                param['jingdianTese'] = jquery('#jingdianTese').val();
            }
                                                                
            // 当前表
                                // 获取列表数据
            http.request('jingdianCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function jingdianTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdian_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.jingdianTypesList = res.data.list;
               }
           });
       }
       ...
       function jingdianDengjiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdian_dengji_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.jingdianDengjiTypesList = res.data.list;
               }
           });
       }
