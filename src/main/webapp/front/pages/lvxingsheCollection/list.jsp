            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">旅行社收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="lvxingsheName" id="lvxingsheName" placeholder="旅行社名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">旅行社星级</label>
                              <div class="layui-input-inline">
                                  <select name="lvxingsheTypes" id="lvxingsheTypes" lay-filter="lvxingsheTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in lvxingsheTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">旅行社位置</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="lvxingsheWeizhi" id="lvxingsheWeizhi" placeholder="旅行社位置" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="lvxingsheJingdian" id="lvxingsheJingdian" placeholder="附近景点" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../lvxingshe/detail.jsp?id='+item.lvxingsheId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.lvxingshePhoto?item.lvxingshePhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../lvxingshe/detail.jsp?id='+item.lvxingsheId)"><span>{{item.lvxingsheName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteLvxingsheCollection(item.id)">删除 {{item.lvxingsheName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 lvxingsheCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            lvxingsheCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteLvxingsheCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('lvxingsheCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#lvxingsheName').val() != null && jquery('#lvxingsheName').val() != "" && jquery('#lvxingsheName').val() != "null" ) {
                param['lvxingsheName'] = jquery('#lvxingsheName').val();
            }
                 
            var lvxingsheTypes = $("#lvxingsheTypes").find("option:selected").val();
            if (lvxingsheTypes != null && lvxingsheTypes != "" && lvxingsheTypes != "null") {
                            param['lvxingsheTypes'] = lvxingsheTypes;
                        }
                 
            if (jquery('#lvxingsheWeizhi').val() != null && jquery('#lvxingsheWeizhi').val() != "" && jquery('#lvxingsheWeizhi').val() != "null" ) {
                param['lvxingsheWeizhi'] = jquery('#lvxingsheWeizhi').val();
            }
                                 
            if (jquery('#lvxingsheJingdian').val() != null && jquery('#lvxingsheJingdian').val() != "" && jquery('#lvxingsheJingdian').val() != "null" ) {
                param['lvxingsheJingdian'] = jquery('#lvxingsheJingdian').val();
            }
                                                
            // 当前表
                                // 获取列表数据
            http.request('lvxingsheCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function lvxingsheTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=lvxingshe_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.lvxingsheTypesList = res.data.list;
               }
           });
       }
