    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑旅行社留言</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑旅行社留言</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">旅行社留言信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>旅行社星级</label>
                                        <input id="lvxingsheValue" name="lvxingsheValue" class="form-control"
                                               placeholder="旅行社星级" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>旅行社位置</label>
                                        <input id="lvxingsheWeizhi" name="lvxingsheWeizhi" class="form-control"
                                               placeholder="旅行社位置" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>旅行社缩略图</label>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>留言内容</label>
                                        <%--<input id="lvxingsheLiuyanContentupload" name="file" type="file">--%>
                                        <script id="lvxingsheLiuyanContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var lvxingsheLiuyanContentUe = UE.getEditor('lvxingsheLiuyanContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="lvxingsheLiuyanContent" id="lvxingsheLiuyanContent-input">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>回复内容</label>
...
<script>
    ...
    function upload() {
        ...
        $('#lvxingsheLiuyanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('lvxingsheLiuyanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("lvxingsheLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addlvxingsheLiuyan');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.lvxingsheLiuyanContent != null && ruleForm.lvxingsheLiuyanContent != 'null' && ruleForm.lvxingsheLiuyanContent != '' /*&& $("#lvxingsheLiuyanContentupload").length>0*/) {

            var lvxingsheLiuyanContentUeditor = UE.getEditor('lvxingsheLiuyanContentEditor');
            lvxingsheLiuyanContentUeditor.ready(function () {
                var mes = '' + ruleForm.lvxingsheLiuyanContent;
                lvxingsheLiuyanContentUeditor.setContent(mes);
                lvxingsheLiuyanContentUeditor.setDisabled('fullscreen');
            });
        }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        // if($("#lvxingsheLiuyanContentupload").length>0) {
            var lvxingsheLiuyanContentEditor = UE.getEditor('lvxingsheLiuyanContentEditor');
            if (lvxingsheLiuyanContentEditor.hasContents()) {
                $('#lvxingsheLiuyanContent-input').attr('value', lvxingsheLiuyanContentEditor.getPlainTxt());
            }
        // }
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                yonghuId: "required",
                lvxingsheLiuyanContent: "required",
                replyContent: "required",
                insertTime: "required",
            },
            messages: {
                lvxingsheId: "旅行社id不能为空",
                yonghuId: "用户id不能为空",
                lvxingsheLiuyanContent: "留言内容不能为空",
    ...
    // 获取当前详情
    function getDetails() {
        var addlvxingsheLiuyan = window.sessionStorage.getItem("addlvxingsheLiuyan");
        if (addlvxingsheLiuyan != null && addlvxingsheLiuyan != "" && addlvxingsheLiuyan != "null") {
            window.sessionStorage.removeItem('addlvxingsheLiuyan');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("lvxingsheLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#lvxingsheLiuyanContent").val(ruleForm.lvxingsheLiuyanContent);
    ...
    function lvxingsheDataBind(){
        ...
        $("#lvxingsheValue").val(ruleForm.lvxingsheValue);
